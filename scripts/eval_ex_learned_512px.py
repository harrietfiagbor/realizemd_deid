"""
eval_ex_learned_512px.py

Same as eval_ex_learned_256px.py but targeting 512x512.
...
Output: eval_ex_learned_512px_results.txt

Run (on pod, GPU):
    python eval_ex_learned_512px.py
"""
import argparse
import cv2
import numpy as np
...
CKPT    = Path(args.ckpt)
LOG     = Path(args.out) if args.out else Path(f"/workspace/eval_ex_learned_512px_{CKPT.parent.name}_results.txt")
...
if __name__ == "__main__":
    import time
    log("Learned EX Mask Quality at 512px — Resampling Mode Comparison")
