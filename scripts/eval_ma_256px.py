...
Tests MA (Microaneurysm) mask quality after downsampling to 256x256.
Uses the learned MA detector (run1, epoch 15, efficientnet-b2, threshold=0.35).
Characterization only, per Adam: does learned-MA survive to working resolution
the way EX/HE were tested, or collapse the way EX did under erosion.

Checkpoint: models/ma_detector/ma_detector_best.pth (run1, epoch 15)

Output: eval_ma_256px_results.txt

Run (on pod, GPU):
    python eval_ma_256px.py
"""
import sys
import cv2
import numpy as np
...
LOG     = Path("/workspace/eval_ma_256px_results.txt")
...
if __name__ == "__main__":
    import time
    log("MA Mask Quality at 256px — Resampling Mode Comparison")
