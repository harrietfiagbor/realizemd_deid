"""
viz_ma_resampling.py

Visualizes learned MA masks at full-res vs 256px vs 512px (maxpool).
Confirms whether MA blobs survive downsampling the way HE does, or collapse like EX.

6 panels per image:
  Full-res GT | Full-res prediction | 256px GT | 256px prediction | 512px GT | 512px prediction

Output: MA_resampling_viz.png

Run (on pod, GPU):
    python viz_ma_resampling.py
"""
import cv2
import numpy as np
import torch
...
OUT     = Path("/workspace/MA_resampling_viz.png")
...
fig.suptitle(
    "MA (Microaneurysms) — Full res vs 256px vs 512px (maxpool), learned detector\n"
